...
export class UserAccount {
  ...
  async createUser(username, password) {
    try {
      ...
      const alreadyRegistered = await userAccountsCollection.findOne({
        userName: username,
      });
  ...
  async sendFriendRequest(senderUsername, receiverUsername) {
    try {
      ...
      senderUsername = exportedMethods.checkString(
        senderUsername,
        "Sender Username"
      );
      receiverUsername = exportedMethods.checkString(
        receiverUsername,
        "Receiver Username"
      );
      const userAccountsCollection = await userAccounts();
      const senderUser = await userAccountsCollection.findOne({
        userName: senderUsername,
      });
      const receiverUser = await userAccountsCollection.findOne({
      ...
      await userAccountsCollection.updateOne(
        { userName: senderUsername },
        { $set: { friendRequests: senderUser.friendRequests } }
      );
    } catch (e) {
      throw new Error(e.message);
    }
  ...
  async acceptFriendRequest(receiverUsername, senderUsername) {
    try {
      ...
      receiverUser.friendRequests = receiverUser.friendRequests.filter(
        (request) => request !== senderUsername
      );
      ...
      senderUser.friendRequests = senderUser.friendRequests.filter(
        (request) => request !== receiverUsername
      );
